...
import ru.valisheva.plugin.composeplugin.feature.features.calendar.injection.CalendarViewFactory

class CalendarAction : AnAction()  {
    override fun actionPerformed(e: AnActionEvent) {
        val deps = ProjectDependencies(e.project)
        val directory = e.getData(CommonDataKeys.PSI_ELEMENT) as PsiDirectory
        val viewModel = CalendarViewFactory.create(directory, deps)
        CalendarDialog(viewModel).show()
    }
}